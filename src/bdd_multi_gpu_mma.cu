...
namespace LPMP {
    ...
    template<typename REAL>
        void bdd_multi_gpu_mma<REAL>::min_marginals_from_directional_costs(const int hop_index, const REAL omega_scalar, thrust::device_ptr<REAL> mm_diff_ptr, const thrust::device_ptr<const REAL> omega_vec)
        {
    ...
    template<typename REAL>
        void bdd_multi_gpu_mma<REAL>::min_marginals_from_directional_costs(const int hop_index, const REAL omega)
        {
    ...
    template<typename REAL>
        void bdd_multi_gpu_mma<REAL>::iteration(const int num_gpus, const size_t max_iter, const double lb_initial, const REAL omega)
        {
            REAL* delta_lo_hi_;
            size_t size_delta_lo_hi_ = this->nr_variables() * 2 * sizeof(REAL);
    ...
    template<typename REAL>
        void bdd_multi_gpu_mma<REAL>::fast_forward_mm(const REAL omega, REAL* delta_lo_hi)
        {
            ...
            fast_compute_delta(this->deffered_mm_diff_.data(), delta_lo_hi);
    ...
    template<typename REAL>
        void bdd_multi_gpu_mma<REAL>::fast_backward_mm(const REAL omega, REAL* delta_lo_hi)
        {
    ...
    template<typename REAL>
        void bdd_multi_gpu_mma<REAL>::fast_compute_delta(
                const thrust::device_ptr<const REAL> mm_to_distribute,
                REAL* delta_lo_hi) const
        {
            //thrust::fill(delta_lo_hi, delta_lo_hi + 2 * this->nr_variables(), 0.0);
            cudaMemset(delta_lo_hi, 0, 2 * this->nr_variables() * sizeof(REAL));
    ...
    template<typename REAL>
        void bdd_multi_gpu_mma<REAL>::compute_delta(
                const thrust::device_ptr<const REAL> mm_to_distribute,
    ...
    template<typename REAL>
        void bdd_multi_gpu_mma<REAL>::fast_normalize_delta(REAL* delta_lo_hi) const
        {
            //assert(delta_lo_hi.size() == 2 * this->nr_variables());
    ...
    template<typename REAL>
        thrust::device_vector<REAL> bdd_multi_gpu_mma<REAL>::net_solver_costs() const
        {
    ...
    template<typename REAL>
        void bdd_multi_gpu_mma<REAL>::flush_mm(thrust::device_ptr<REAL> mm_diff_ptr)
    ...
    template class bdd_multi_gpu_mma<float>;
    template class bdd_multi_gpu_mma<double>;
}
